...
int search_nearest(map<int, Point> &m, char* visited, Point given) {
  int key = -1;
  float min = INF;
...
int search_about_near(map<int, Point> &m, char* visited, int given) {
  int key = -1;
  int secondkey = -1;
  float min = INF;
  float secondmin = INF;
...
int* make_tour(int* order, map<int, Point> &m, int start) {
  ...
  for (int i = 0; i < num-1; i++) {
    order[i] = start;
    visited[start] = 1;
    next = search_about_near(m, visited, start);
    start = next;
    }
  order[num-1] = next;
...
int main(int argc, char* argv[]) {
  ...
  int start = search_nearest(tour, dum, zero);
  float length = calculate_tour(tour, make_tour(order, tour, start));
